...
public class UserRegistration extends javax.swing.JFrame {
    ...
    public void loadUserType() {
        try {
            ResultSet rs = MySQL.search("SELECT * FROM `user_type` ORDER BY `name` ASC");
            Vector v = new Vector();
    ...
    public void loadUserTable() {
        try {
            ...
            dtm.setRowCount(0);
            ResultSet rs = MySQL.search("SELECT * FROM `user` ORDER BY `name` ASC");
            while (rs.next()) {
                Vector v = new Vector();
                ...
                rc.next();
                ResultSet rut = MySQL.search("SELECT * FROM `user_type` WHERE `id`='" + rs.getString("user_type_id") + "'");
                rut.next();
    ...
    public void loadSearchSupplierTable() {
        try {
            DefaultTableModel dtm = (DefaultTableModel) jTable1.getModel();
            dtm.setRowCount(0);
            ResultSet rs = MySQL.search("SELECT * FROM `user` WHERE `name` LIKE '%" + jTextField1.getText() + "%' AND `contact_no` LIKE '%" + jTextField2.getText() + "%' AND `email` LIKE '%" + jTextField3.getText() + "%' ORDER BY `name` ASC");
            while (rs.next()) {
                Vector v = new Vector();
                ResultSet ra = MySQL.search("SELECT * FROM `address` WHERE `id`='" + rs.getString("address_id") + "'");
                ra.next();
    ...
    private void initComponents() {
                            ...
                            .addGroup(jPanel2Layout.createSequentialGroup()
                                ...
                                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    ...
                                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel2Layout.createSequentialGroup()
                                        ...
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                            .addComponent(jTextField3, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.PREFERRED_SIZE, 431, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel2Layout.createSequentialGroup()
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "NAME", "USERNAME", "PASSWORD", "CONTACT NO", "EMAIL", "ADDRESS", "USER TYPE", "STATUS"
            }
        ) {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (jTextField1.getText().isEmpty()) {
            ...
            JOptionPane.showMessageDialog(this, "Please Enter User Contact No", "Warning", JOptionPane.WARNING_MESSAGE);
            ...
            JOptionPane.showMessageDialog(this, "Please Enter User Username", "Warning", JOptionPane.WARNING_MESSAGE);
        } else if (password.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please Enter User Password", "Warning", JOptionPane.WARNING_MESSAGE);
        } else if (password.length() < 8) {
            JOptionPane.showMessageDialog(this, "Password Should Contain Athleast 8 Characters", "Warning", JOptionPane.WARNING_MESSAGE);
        } else if (jComboBox2.getSelectedIndex() == 0) {
            JOptionPane.showMessageDialog(this, "Please Select User User-type", "Warning", JOptionPane.WARNING_MESSAGE);
        } else if (jTextField5.getText().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please Enter User Street Name", "Warning", JOptionPane.WARNING_MESSAGE);
        } else if (jComboBox1.getSelectedIndex() == 0) {
            JOptionPane.showMessageDialog(this, "Please Select User City", "Warning", JOptionPane.WARNING_MESSAGE);
        } else {
            if (jButton1.getText().equals("Register User")) {
                try {
                    ResultSet check = MySQL.search("SELECT * FROM `user` WHERE `username`='" + jTextField6.getText() + "' AND `email`='" + jTextField3.getText() + "' AND `email`='" + jTextField3.getText() + "'");
                    if (check.next()) {
                        ...
                        ResultSet rut = MySQL.search("SELECT * FROM `user_type` WHERE `name`='" + jComboBox2.getSelectedItem() + "' ");
                        rut.next();
                        MySQL.iud("INSERT INTO `user`(`name`,`username`,`password`,`contact_no`,`email`,`user_type_id`,`status_id`,`address_id`) VALUES('" + jTextField1.getText() + "','" + jTextField6.getText() + "','" + password + "','" + jTextField2.getText() + "','" + jTextField3.getText() + "','" + rut.getString("id") + "','1','" + id + "')");
                        resetFields();
                ...
                try {
                    ResultSet check = MySQL.search("SELECT * FROM `user` WHERE `username`='" + jTextField6.getText() + "' AND `email`='" + jTextField3.getText() + "' AND `id`<>'" + jLabel9.getText() + "'");
                    if (check.next()) {
                        JOptionPane.showMessageDialog(this, "User Already Registered", "Warning", JOptionPane.WARNING_MESSAGE);
                    } else {
                        ResultSet rc = MySQL.search("SELECT * FROM `city` WHERE `name`='" + jComboBox1.getSelectedItem().toString() + "'");
                        ...
                        MySQL.iud("UPDATE `user` SET `name`='" + jTextField1.getText() + "',`address_id`='" + id + "',`contact_no`='" + jTextField2.getText() + "',`email`='" + jTextField3.getText() + "' WHERE `id`='" + jLabel9.getText() + "'");
                        resetFields();
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        ...
        if (evt.getClickCount() == 2) {
            if (SignIn.user_type_id == 1 || SignIn.user_type_id == 2) {
                try {
                    if (jTable1.getValueAt(sr, 8).equals("Active")) {
                        if (jTable1.getValueAt(sr, 7).equals("Admin")) {
                            JOptionPane.showMessageDialog(this, "An Admin Cannot Be Blocked", "Warning", JOptionPane.WARNING_MESSAGE);
                        } else {
                            MySQL.iud("UPDATE `user` SET `status_id`='2' WHERE `id`='" + jTable1.getValueAt(sr, 0) + "'");
                            JOptionPane.showMessageDialog(this, "User Blocked Successfully", "Success", JOptionPane.INFORMATION_MESSAGE);
                        }
                    } else {
                        MySQL.iud("UPDATE `user` SET `status_id`='1' WHERE `id`='" + jTable1.getValueAt(sr, 0) + "'");
                        JOptionPane.showMessageDialog(this, "User Unblocked Successfully", "Success", JOptionPane.INFORMATION_MESSAGE);
                    }
                    resetFields();
                ...
                JOptionPane.showMessageDialog(this, "Only Admin or A General Manager Can Disabled a User", "Warning", JOptionPane.WARNING_MESSAGE);
            }
        } else if (evt.getClickCount() == 1) {
            jLabel8.setText("Id :");
            jLabel9.setText(jTable1.getValueAt(sr, 0).toString());
            jTextField1.setText(jTable1.getValueAt(sr, 1).toString());
            jTextField2.setText(jTable1.getValueAt(sr, 4).toString());
            jTextField3.setText(jTable1.getValueAt(sr, 5).toString());
            String address_ct = jTable1.getValueAt(sr, 6).toString();
            String[] address = address_ct.split(", ");
            jTextField4.setText(address[0]);
            jTextField5.setText(address[1]);
            jTextField6.setText(jTable1.getValueAt(sr, 2).toString());
            jTextField6.setEditable(false);
            jPasswordField1.setText(jTable1.getValueAt(sr, 3).toString());
            jPasswordField1.setEditable(false);
            jComboBox2.setSelectedItem(jTable1.getValueAt(sr, 7).toString());
